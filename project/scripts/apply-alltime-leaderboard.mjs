/**
 * Apply all-time leaderboard migration to remote Supabase Postgres.
 *
 * Recommended: Supabase Dashboard → SQL Editor → apply_alltime_leaderboard.sql
 *
 * CLI: SUPABASE_DB_PASSWORD=... npm run db:apply-alltime
 */
...
if (!process.env.SUPABASE_DB_URL && (!projectRef || !password || PLACEHOLDER.test(password))) {
  ...
  console.error('\nOption A: Supabase Dashboard → SQL Editor → run apply_alltime_leaderboard.sql');
  console.error('Option B: SUPABASE_DB_URL=... npm run db:apply-alltime');
  console.error('Option C: SUPABASE_DB_PASSWORD=... npm run db:apply-alltime');
  process.exit(1);
}

const sqlPath = path.join(__dirname, '..', 'supabase', 'apply_alltime_leaderboard.sql');
...
for (const connectionString of buildConnectionCandidates()) {
  ...
  try {
    ...
    console.log('Applying all-time leaderboard migration …');
...
console.error('\nUse Supabase Dashboard → SQL Editor → apply_alltime_leaderboard.sql');
process.exit(1);
